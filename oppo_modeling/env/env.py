...
def get_obs(infoset, init_card):   # Add init_card for inferring the true hand cards of next player
...
def _cards2array(list_cards):              # 把手牌转换成1*54维的矩阵
    if len(list_cards) == 0:
...
def legal_predict(other_card):      # Encode the leagl label
...
def get_label(other_card):  # Encode the true label of next player's cards
    counter = Counter(other_card)
    matrix = [0] * 15
    for card, num_times in counter.items():
        if card < 20:
...
def test_get_obs(infoset):       # Used for test and not contain cards of next player.
    if infoset.player_position == 'landlord':
