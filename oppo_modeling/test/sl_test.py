...
def judge_test():
    p = os.popen("ps aux | grep baseline_sl | grep -v grep | awk '{print $2}' ")
    x = p.read()
    print(x)
    p.close()
    if not x:        
...
if __name__ == '__main__':
    ...
    log_path = os.path.join(log_dir, 'sl_test.log')
    csv_path = os.path.join(log_dir, 'sl_eval.csv')
    ...
    os.system("".join([
        ...
        " --landlord ",
        landlord_ckpt,
        " --landlord_up baseline_sl",
        " --landlord_down baseline_sl",
        " --eval_data ",
        eval_data_path,
        " --csv_path ",
        csv_path,
