...
SEAT_ORDER = ['landlord', 'landlord_down', 'landlord_up']
OPPONENT_ORDER = {
...
def get_obs(infoset, init_card):
    ...
    if infoset.player_position == 'landlord_up':
        return _get_obs_landlord_up(infoset, init_card)
    if infoset.player_position == 'landlord_down':
...
def _split_action_seq_by_position(action_seq):
    per_position = {pos: [] for pos in SEAT_ORDER}
    for idx, action in enumerate(action_seq):
        per_position[SEAT_ORDER[idx % len(SEAT_ORDER)]].append(action)
    return per_position


def _get_opponent_histories(infoset):
    per_position = _split_action_seq_by_position(infoset.card_play_action_seq)
    opponent_positions = OPPONENT_ORDER[infoset.player_position]
    opponent_histories = []
    for opponent in opponent_positions:
        seq = _process_action_seq(per_position[opponent])
        opponent_histories.append(_action_seq_list2array(seq))
    return np.stack(opponent_histories, axis=0)
...
def _get_obs_landlord(infoset, init_card):
    ...
    z_batch = np.repeat(
        z[np.newaxis, :, :],
        num_legal_actions, axis=0)
    opp_z = _get_opponent_histories(infoset)
    obs = {
        'position': 'landlord',
